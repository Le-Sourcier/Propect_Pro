...
import { Plus, Send, Mail, Clock, Edit, Trash, Eye, BarChart3, Search, Filter, ChevronDown, X } from 'lucide-react';
...
const EmailCampaigns = () => {
  ...
  const [templates] = useState<Template[]>([
    ...
    },
    {
      ...
      preview: 'Hi {{first_name}}, I would like to invite you to a personalized demo...',
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredCampaigns.map((campaign) => (
                    <tr key={campaign.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" title={campaign.subject}>
                        {campaign.subject.length > 30 ? `${campaign.subject.substring(0, 30)}...` : campaign.subject}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          {campaign.status === 'sent' ? (
                            <button 
                              onClick={() => navigate(`/email-campaigns/${campaign.id}/analytics`)}
                            ...
                            <button 
                              onClick={() => navigate(`/email-campaigns/${campaign.id}/edit`)}
                              className="text-blue-600 hover:text-blue-900"
                              title="Edit Campaign"
                            >
              ...
              {templates.map((template) => (
                <div key={template.id} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                  ...
                  <div className="bg-gray-50 px-4 py-4 sm:px-6 flex justify-between">
                    <button 
                      onClick={() => navigate(`/email-campaigns/templates/${template.id}/edit`)}
        ...
        {activeTab === 'contacts' && (
          <div className="p-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
              <ul className="divide-y divide-gray-200">
                {contactLists.map((list) => (
                  <li key={list.id} className="py-4 px-6 flex items-center justify-between">
                    <div>
                    ...
                    <div className="flex space-x-2">
                      <button 
                        onClick={() => navigate(`/email-campaigns/contacts/${list.id}`)}
                        className="text-blue-600 hover:text-blue-900"
                        title="View Contacts"
                      >
                        <Eye className="h-4 w-4" />
            ...
            <div className="mt-6 bg-white border border-gray-200 rounded-lg p-6">
              <h3 className="text-sm font-medium text-gray-900 mb-4">GDPR Compliance</h3>
              <div className="space-y-3">
                <div className="flex items-start">
                  ...
                  <div className="ml-3 text-sm">
                    <label htmlFor="unsubscribe" className="font-medium text-gray-700">
                      Unsubscribe Link
                    </label>
                    <p className="text-gray-500">Automatically include an unsubscribe link in all emails</p>
                  </div>
                </div>
                <div className="flex items-start">
                  <div className="h-5 flex items-center">
                  ...
                  <div className="ml-3 text-sm">
                    <label htmlFor="tracking" className="font-medium text-gray-700">
                      Tracking Disclosure
                    </label>
                    <p className="text-gray-500">Include a disclosure about email tracking</p>
                  </div>
                </div>
                <div className="flex items-start">
                  <div className="h-5 flex items-center">
                  ...
                  <div className="ml-3 text-sm">
                    <label htmlFor="optout" className="font-medium text-gray-700">
                      Maintain Opt-out List
                    </label>
                    <p className="text-gray-500">Automatically update and honor opt-out requests</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {contactLists.map(list => (
                    <option key={list.id} value={list.id}>{list.name} ({list.count} contacts)</option>
                  ))}
                </select>
              </div>
