...
module.exports = {
  ...
  // src/controllers/components/jobController.js
  ...
  updateJob: async (req, res) => {
    try {
      ...
      const updates = req.body;
      const job = await Jobs.findByPk(jobId);
      if (!job) return serverMessage(res, "JOB_NOT_FOUND");

      // Si on passe en "running", lancez le scraper sans attendre
      if (updates.status === "running" && job.status !== "running") {
        GoogleScraper(job.query, job.location)
          .then(async (results) => {
            // mettez à jour le job en base
            await job.update({ status: "completed", results });
            // NOTIFIEZ les clients SSE
            // jobEmitter.emit(jobId, { status: "completed", results });
          })
          .catch(async (err) => {
            await job.update({ status: "failed" });
            // jobEmitter.emit(jobId, { status: "failed" });
          });
      }

      // Préservez les champs non modifiables
      ...
      // Appliquez la mise à jour souhaitée (status, etc.)
      await job.update(updates);

      // Émettez un événement immédiat (p. ex. changement de status)
      jobEmitter.emit(jobId, {
        status: job.status,
        results: job.results,
      });
