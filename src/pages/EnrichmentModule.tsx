...
import { Upload, Database, Download, Filter, RefreshCw, FileText } from 'lucide-react';
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-200">
          <div className="flex flex-wrap items-center justify-between">
            <h1 className="text-2xl font-semibold text-gray-900">Data Enrichment</h1>
          ...
          <div className="px-6 py-5">
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">Drop your file here</h3>
            ...
            <div className="mt-6 bg-gray-50 p-4 rounded-md border border-gray-200">
              ...
              <ul className="text-xs text-gray-600 space-y-1">
                ...
                <li>• Recommended: Include company names for better matching</li>
              </ul>
            </div>

            <div className="mt-6">
              <h3 className="text-sm font-medium text-gray-700 mb-4">Select Data Sources for Enrichment</h3>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {dataSources.map((source) => (
                  <div key={source.id} className="relative flex items-start p-4 bg-white rounded-lg border border-gray-200">
                    ...
                    <div className="ml-3 flex-1">
                      <div className="flex items-center justify-between">
                        <label htmlFor={source.id} className="font-medium text-gray-700 text-sm">
                          {source.name}
                        </label>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                          source.statusColor === 'green' ? 'bg-green-100 text-green-800' :
                          source.statusColor === 'yellow' ? 'bg-yellow-100 text-yellow-800' :
                          'bg-gray-100 text-gray-800'
                        }`}>
                          {source.status}
                        </span>
                      </div>
                      <p className="text-gray-500 text-xs mt-1">{source.description}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {enrichmentJobs.map((job) => (
                  <tr key={job.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {job.enriched} ({Math.round((job.enriched / job.records) * 100) || 0}%)
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        ...
        {activeTab === 'sources' && (
          <div className="px-6 py-5">
            <div className="space-y-6">
              {dataSources.map((source) => (
                <div key={source.id} className="bg-gray-50 p-4 rounded-md border border-gray-200">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-sm font-medium text-gray-700">{source.name}</h3>
