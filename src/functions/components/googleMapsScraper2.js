...
// Build reasult object
async function buildResult(page, isArray = false) {
  if (isArray)
...
async function Scraper(name, location) {
  ...
  const results = await buildResult(page, (isArray = true));
...
async function Enricher(name, location) {
  ...
  // Extract data from the loaded search results
  const result = await await buildResult(page, (isArray = false));
